# Project 7 - How does invertebrate composition change with elevation.
# Lubomir - Adelas version
library(tidyverse)
install.packages('DHARMa')
install.packages('vegan')
library(vegan)

#bada <- read.csv("badaguish.csv")
...
theme_coding <- function(){
  theme_bw() +
    theme(axis.text.x = element_text(size = 20, vjust = 1, hjust = 0.5, colour = "black"),
...
# Plot model
(prich.m <- ggplot(meall, aes(x = elev_m, y = plant_sp_rich)) +
    geom_point(colour = "#36648B") +
    geom_smooth(method = glm, colour = "#36648B", fill = "#4F94CD", alpha = 0.6) +
    scale_x_continuous(breaks = c(400, 500, 600, 700, 800)) +
    theme_coding() +
    labs(x = "Elevation (m)", y = "Species Richness"))

#quadrat_data_long <- read.csv("quadrat_data_long.csv")


# let's do the graph of moss and inv diversity
# the model is lm_div_moss; linear model, normal distribution
shannon_diversity ~ moss_cov
lm_div_moss <- lm(shannon_diversity ~ moss_cov, data = meall)

ggplot(meall, aes(x = moss_cov, y = shannon_diversity)) +
  geom_smooth(method = lm, linewidth = 3, alpha = 0.2, colour = "#85aae4", fill = "#85aae4") +
  geom_point(size = 5, colour = "#735b3e") +
  labs(x = "\nMoss Cover (%)", y = "Invertebrate Shannon Diversity\n") +
  theme_coding()
...
summary(lm_div_moss) # p = 0.0024, t =  3.529
anova(lm_div_moss)


## Question 1 - Invertebrate composition changes with elevation-------------------------------
...
lm_rich_elev <- lm(inv_order_rich ~ elev_m, data = meall)
summary(lm_rich_elev) # intercept significant, elevation not

# Higher richness can be found in mid altitudes, based on this model
...
plot(shannon_diversity ~ moss_cov, data = meall) # see beginning of script

# INVERTEBRATE ABUNDANCE WITH ELEVATION AND WITH MOSS COVER
# As in question 3, elevation explains some variance in abundance, however, moss cover
# is a better predictor.
# Actually no, all models are over-dispersed (Lubos)

plot(inv_abun ~ elev_m, data = meall) # decreasing
abline(lm(inv_abun ~ elev_m, data=meall))

plot(inv_abun ~ moss_cov, data = meall) # increasing
abline(lm(inv_abun ~ moss_cov, data=meall))

glm_abun_elev <- glm(inv_abun ~ elev_m, data = meall, family = poisson)
summary(glm_abun_elev) # this seems to be pretty significant and not overdispersed

glm_abun_moss <- glm(inv_abun ~ moss_cov, data = meall, family = poisson)
summary(glm_abun_moss) #also significant and not overdisperser (overly)

## Question 2 - plants affected by elevation ------------------------------

# plant richness ~ elevation
ggplot(meall, aes(x = elev_m, y = plant_sp_rich)) +
  geom_point() +
  geom_smooth(method = lm) +
  theme_classic()

speglm <- glm(plant_sp_rich ~ elev_m, data = meall, family = poisson)
shapiro.test(resid(speglm))  # not significant but nearly lol
plot(speglm)

plot(plant_sp_rich ~ elev_m, data = meall)
abline(lm(plant_sp_rich ~ elev_m, data = meall))

summary(speglm) #intercept significant, elev not

# heather cover ~ elevation
ggplot(meall, aes(x = elev_m, y = heather_cov)) +  # can't exclude first plot, not good enough reasons
   geom_point() +
  geom_smooth(method = lm) +
  theme_classic()

lm_heather_elev <- lm(heather_cov~elev_m, data=meall)
shapiro.test(resid(lm_heather_elev)) #ok, not sig.
summary(lm_heather_elev) # not significant

# moss cover ~ elevation
ggplot(meall, aes(x = elev_m, y = moss_cov)) +
  geom_point() +
  geom_smooth(method = lm) +
  theme_classic()

lm_moss_elev <- lm(moss_cov~elev_m, data=meall)
summary(lm_moss_elev) # not significant

shapiro.test(resid(lm_moss_elev))  # good
plot(lm_moss_elev)  # wierd shit going on with Scale-location plots

# Look at data distribution in moss cover
hist(meall$moss_cov)
hist(log(meall$moss_cov))

# WHY
par(mfrow = c(2,2))
lm <- lm(log(moss_cov+1) ~ elev_m, data = meall)
summary(lm)  # <<<<<<<<<<<<<<<<<<<<<<<<<used this one
lm0 <- lm(log(moss_cov+1) ~ 1, data = meall)
...
plot(log(moss_cov+1) ~ elev_m, data = meall)
abline(lm(log(moss_cov+1) ~ elev_m, data = meall))

#do the same for heather
hist(meall$heather_cov)
lm2 <- lm(log(heather_cov+1) ~ elev_m, data = meall)
summary(lm2) #not significant

# lichen ~ elevation
ggplot(meall, aes(x = elev_m, y = lichen_cov)) +
  geom_point() +
  geom_smooth(method = lm) +
  theme_classic()

# grass ~ elevation
ggplot(meall, aes(x = elev_m, y = grass_cov)) +
  geom_point() +
  geom_smooth(method = lm) +
  theme_classic()
...
hist(meall$heather_cov)
hist(meall$moss_cov)
hist(meall$grass_cov)
hist(meall$lichen_cov)

# transform heather
meall$heather_log <- log(meall$heather_cov)
meall$heather_sqrt <- sqrt(meall$heather_cov)
hist(meall$heather_log)
hist(meall$heather_sqrt)  # probably better
# heather lm
lm_heat <- lm(heather_cov ~ elev_m, data = meall)
shapiro.test(resid(lm_heat))  # not significant
plot(lm_heat) #residuals vs leverage plot is p bad
summary(lm_heat) #not significant


lm_heather <- lm(heather_sqrt ~ elev_m, data = meall)
lm_heather_log <- lm(heather_log ~ elev_m, data = meall)
par(mfrow = c(2,2))
plot(lm_heather)
plot(lm_heather_log)
summary(lm_heather_log)  # <<<<<<<<<<<<<<<<<<< used this one

# transform lichen
meall$lichen_log <- log(meall$lichen_cov)
meall$lichen_sqrt <- sqrt(meall$lichen_cov)

par(mfrow=c(1,1))
hist(meall$lichen_log)
hist(meall$lichen_sqrt)

meall$moss_log <- log(meall$moss_cov) + 1
hist(meall$moss_log)

chisq.test(meall$elev_m, meall$moss) #significant


# Does total plant cover percentage change with elevation?
meall$total_cov <- meall$heather_cov + meall$moss_cov + meall$grass_cov + meall$lichen_cov
hist(meall$total_cov)
lm_totcov <- lm(total_cov ~ elev_m, data = meall)
totcovresid <- resid(lm_totcov)
shapiro.test(totcovresid)  # normality great
par(mfrow = c(2,2))
plot(lm_totcov)
summary(lm_totcov)  # overdispersed

# try log-transformation
meall$total_cov_log <- log(meall$total_cov)
hist(meall$total_cov_log)
lm_totcovlog <- lm(total_cov_log ~ elev_m, data = meall)
totcovlogresid <- resid(lm_totcovlog)
shapiro.test(totcovlogresid)  # normality good
par(mfrow = c(2,2))
plot(lm_totcovlog)
summary(lm_totcov)  # still overdispersed
...
shapiro.test(resid(lm_richlog_moss))  # good
...
shapiro.test(resid(glm_rich_moss))  # good
...
# Both passed Shapiro test, diagnostics of poisson model are slightly better.
# But isn't the argument that AIC of the log model is better than null model enough? I'd guess it is.
par(mfrow = c(1,1))
...
moss_resid <- resid(lm_div_moss)
shapiro.test(moss_resid)  # normality is good

# conclusions: moss coverage seems to be the best explanation for inv diversity
# heather had an insignificant effect.
# use moss

plot(lm_div_heather)
summary(lm_div_heather)  
...
# they are over-dispersed. Let's do quasipoisson
glm_abu_null2 <- glm(inv_abun ~ 1, data = meall, family = quasipoisson)
glm_abu_el2 <- glm(inv_abun ~ elev_m, data = meall, family = quasipoisson)
glm_abu_pl2 <- glm(inv_abun ~ plant_sp_rich, data = meall, family = quasipoisson)
glm_abu_elpl2 <- glm(inv_abun ~ elev_m + plant_sp_rich, data = meall, family = quasipoisson)
glm_abu_elplint2 <- glm(inv_abun ~ elev_m * plant_sp_rich, data = meall, family = quasipoisson)
AIC(glm_abu_null2, glm_abu_el2, glm_abu_pl2, glm_abu_elpl2, glm_abu_elplint2)
AICc(glm_abu_null2, glm_abu_el2, glm_abu_pl2, glm_abu_elpl2, glm_abu_elplint2)

summary(glm_abu_el2)
# there are no AIC values, elevation is not significant anymore.
...
# overdispersed, do quasipoisson
glm_abu_heather2 <- glm(inv_abun ~ heather_cov, data = meall, family = quasipoisson)
glm_abu_moss2 <- glm(inv_abun ~ moss_cov, data = meall, family = quasipoisson)
glm_abu_heatmoss2 <- glm(inv_abun ~ heather_cov + moss_cov, data = meall, family = quasipoisson)
glm_abu_heatmossint2 <- glm(inv_abun ~ heather_cov * moss_cov, data = meall, family = quasipoisson)
glm_abu_elmoss2 <- glm(inv_abun ~ elev_m + moss_cov, data = meall, family = quasipoisson)
glm_abu_elmossint2 <- glm(inv_abun ~ elev_m * moss_cov, data = meall, family = quasipoisson)
AIC(glm_abu_null2, glm_abu_heather2, glm_abu_moss2, glm_abu_heatmoss2, glm_abu_heatmossint2,
    glm_abu_elmoss2, glm_abu_elmossint2, glm_abu_el2)
summary(glm_abu_moss2) # still significant
...
summary(lm_richlog_moss)
summary(lm_div_moss)
summary(glm_abu_moss2)  # doesn't show F value
anova(glm_abu_moss2)  # 1,18 probably

plot(lm_richlog_moss)
plot(lm_div_moss)
plot(glm_abu_moss2)

par(mfrow = c(2,2))
# richness
plot(inv_order_rich ~ moss_cov, data = meall)
abline(lm(inv_order_rich ~ moss_cov, data = meall))
# div
plot(shannon_diversity ~ moss_cov, data = meall)
abline(lm(shannon_diversity ~ moss_cov, data = meall))
# abun
plot(inv_abun ~ moss_cov, data = meall)
abline(lm(inv_abun ~ moss_cov, data = meall))

anova(lm(inv_abun ~ moss_cov, data = meall))
...
NMDS2 <- metaMDS(inv, k = 3, trymax = 100, trace = F, autotransform = FALSE)
NMDS2
