...
setwd("C:/Users/hp/Desktop/Here we go!/EES field course (2023-24)/analysis")
...
# Actually no, all models are over-dispersed.
...
spe <- lm(plant_sp_rich ~ elev_m, data = bada)
shapiro.test(resid(spe))  # it's significant
hist(bada$plant_sp_rich)  # that might be it, try poisson
par(mfrow = c(2,2))
plot(spe)

speglm <- glm(plant_sp_rich ~ elev_m, data = bada, family = poisson)
shapiro.test(resid(speglm))  # not significant
plot(speglm)
summary(speglm)
anova(speglm)
...
mlm <- lm(moss_cov ~ elev_m, data = bada)
shapiro.test(resid(mlm))  # good
plot(mlm)  # strange
summary(mlm)

hist(bada$moss_cov)
hist(log(bada$moss_cov))
...
lm2 <- lm(log(heather_cov+1) ~ elev_m, data = bada)
summary(lm2)
...
# heather lm
lm_heat <- lm(heather_cov ~ elev_m, data = bada)
